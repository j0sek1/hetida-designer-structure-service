...
from hetdesrun.persistence.structure_service_dbmodels import ElementTypeDBModel
...
def fetch_element_types(
    ...
    """
    Fetches ElementTypeDBModel records from the database based on stakeholder_key and external_id.
    ...
    """
    existing_ets_mapping: dict[tuple[str, str], ElementTypeDBModel] = {}
    if not keys:
    ...
    try:
        ...
        logger.debug(
            "Fetched %d ElementTypeDBModel items from the database.", len(existing_ets_mapping)
        )
        return existing_ets_mapping
...
def search_element_types_by_name(
...
) -> list[ElementTypeDBModel]:
    """
    Searches for ElementTypeDBModel records based on a partial or full name match.
    ...
    try:
        element_types = (
            ...
            .filter(ElementTypeDBModel.name.ilike(f"%{name_query}%"))
            .all()
        )
        logger.debug(
            "Found %d ElementTypeDBModel items matching name query '%s'.",
            len(element_types),
            name_query,
        )
        return element_types
    except IntegrityError as e:
        logger.error("Integrity Error while searching ElementTypeDBModel by name: %s", e)
        raise DBIntegrityError("Integrity Error while searching ElementTypeDBModel by name") from e
    except Exception as e:
        logger.error("Unexpected error while searching ElementTypeDBModel by name: %s", e)
        raise DBError("Unexpected error while searching ElementTypeDBModel by name") from e
...
def upsert_element_types(
    ...
    """
    Upserts ElementTypeDBModel records using SQLAlchemy's merge and add functionalities.
    ...
    try:
        ...
        with session.no_autoflush:
            for element in elements:
                ...
                if db_element:
                    logger.debug("Updating ElementTypeDBModel with key %s.", key)
                    # Update fields
                    ...
                    logger.debug("Creating new ElementTypeDBModel with key %s.", key)
                    # Create a new ElementTypeDBModel object
                    new_element = ElementTypeDBModel(
                        id=element.id,
        ...
        logger.error("Integrity Error while upserting ElementTypeDBModel: %s", e)
        raise DBIntegrityError("Integrity Error while upserting ElementTypeDBModel") from e
    except Exception as e:
        logger.error("Error while upserting ElementTypeDBModel: %s", e)
        raise DBUpdateError("Error while upserting ElementTypeDBModel") from e
