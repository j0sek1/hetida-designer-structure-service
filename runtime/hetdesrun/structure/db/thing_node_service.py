...
def fetch_thing_nodes(
    ...
    Returns:
        Dict[Tuple[str, str], ThingNodeOrm]: A mapping from (stakeholder_key, external_id) to ThingNodeOrm.
...
def upsert_thing_nodes(
    ...
    try:
        ...
        with session.no_autoflush:
            for node in thing_nodes:
                ...
                if not element_type:
                    ...
                    logger.warning(
                        "ElementType with key (%s, %s) not found for ThingNode %s. Skipping update.",
        ...
        session.flush()  # Explicitly flush all changes at once to ensure data is written to the database.
