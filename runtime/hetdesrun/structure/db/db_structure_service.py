...
def orm_load_structure_from_json_file(file_path: str) -> CompleteStructure:
    ...
    try:
        ...
        # If there is a TypeError while converting JSON data to CompleteStructure, raise a DBParsingError
...
def sort_thing_nodes(
    ...
    """
    Sorts ThingNodes into hierarchical levels and flattens the structure, excluding orphan nodes (nodes without valid parent).

    Args:
        thing_nodes (list[ThingNode]): The ThingNodes to sort.
        existing_thing_nodes (dict[tuple[str, str], ThingNodeOrm]): Existing ThingNodes from the database.
    ...
    for tn in thing_nodes:
        if tn.parent_external_node_id:
            ...
            if parent_tn:
                ...
                logger.warning(
                    "Orphan node detected: Parent ThingNode with key %s not found for ThingNode %s. Excluding from sort.",
                    parent_key,
                    tn.name,
                )
        else:
...
def populate_element_type_ids(
    ...
    Args:
        ...
        existing_element_types (dict[tuple[str, str], ElementTypeOrm]): Existing ElementTypes from the database.
    """
...
def orm_delete_structure(session: SQLAlchemySession) -> None:
    ...
    This function ensures that records are deleted in the correct order to maintain referential integrity.
