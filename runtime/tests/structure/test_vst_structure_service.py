...
def test_update_structure_vsta(mocked_clean_test_db_session):
    ...
    with mocked_clean_test_db_session() as session:
        ...
        # Check if the 'Energy consumption of a single pump in Storage Tank' Source was correctly inserted
        source = next(
        ...
        # Check if the 'Anomaly Score for the energy usage of the pump system in Storage Tank' Sink was correctly inserted
        sink = next(
            (
                s
                for s in sinks
